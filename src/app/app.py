...
from src.models.model_utils import load_vocabs, load_model, load_cross_encoder
from src.models.predict import get_recommendations
import src.config as config

app = Flask(__name__)

# We will attach artifacts to the app object instead of using globals
app.DUAL_ENCODER = None
...
def load_artifacts(flask_app):
    ...
    flask_app.COFFEE_DF = pd.read_csv(config.PREPROCESSED_DATA_PATH)
    # flask_app.COFFEE_DF["combined_text"] = flask_app.COFFEE_DF["blind assessment"].fillna("") + " " + flask_app.COFFEE_DF["bottom line"].fillna("")
    
    print("Loading Model...")
    flask_app.VOCABS = load_vocabs(config.VOCABS_PATH)
    flask_app.DUAL_ENCODER = load_model(
        vocabs=flask_app.VOCABS,
        device=device,
        eval=True
    )
    flask_app.CROSS_ENCODER = load_cross_encoder(
        model_arch_path=config.CE_ARCHITECTURE,
        weights_path=config.CE_WEIGHTS,
        device=device,
        eval=True
    )
    ...
    flask_app.FAISS_INDEX = faiss.read_index(str(config.FAISS_INDEX_PATH))
...
def recommend():
    ...
    if not app.DUAL_ENCODER or not app.FAISS_INDEX or app.COFFEE_DF.empty or not app.CROSS_ENCODER:
    ...
    try:
        ...
        recommendations = get_recommendations(
            query=query, 
            dual_encoder=app.DUAL_ENCODER, 
            cross_encoder=app.CROSS_ENCODER,
            faiss_index=app.FAISS_INDEX, 
            coffee_df=app.COFFEE_DF, 
            initial_k=50,
            final_k=10
        )
